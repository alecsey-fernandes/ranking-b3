...
- **Fase 2 — preço via B3 (COTAHIST)**: pelo mesmo motivo (Brapi paga),
  ...
  de mercado: B3" acima.
- **Correção de memória no cliente B3**: a primeira versão do
  `b3_cotahist_client.py` carregava o arquivo anual inteiro (que cobre
  TODOS os papéis negociados no ano) em memória antes de processar,
  causando um 502 em produção por estouro de RAM (o processo reiniciava
  sozinho no meio da requisição). Reescrito para baixar em streaming
  direto pro disco e processar linha a linha, sem nunca materializar o
  arquivo inteiro em memória — ver `_baixar_zip_ano` e
  `_iterar_linhas_do_zip`.
- **Correção de URL/bloqueio no cliente B3**: a URL usada era `http://`
  (sem criptografia) e sem cabeçalho `User-Agent`, o que resultava em
  erro 520 (bloqueio da Cloudflare, que protege o site da B3, contra
  requisições que não parecem vir de um navegador). Corrigido para
  `https://` com um `User-Agent` de navegador comum.
